...
const GalleryAdmin = () => {
  ...
  // Load gallery items from backend (GET /leadadmin/gallery)
  useEffect(() => {
    const load = async () => {
      try {
        console.log("[GalleryAdmin] GET /leadadmin/gallery - loading...");
        const res = await api.get("/leadadmin/gallery");
        console.log("[GalleryAdmin] GET /leadadmin/gallery response", res?.status, res?.data);
        ...
        console.error("[GalleryAdmin] GET /leadadmin/gallery error", error?.message, error?.response?.data);
      }
    };
    load();
  }, []);
