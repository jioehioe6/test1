...
const HeroAdmin = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        console.log("[HeroAdmin] GET /leadadmin/all-content - loading banner images...");
        const res = await api.get("/leadadmin/all-content");
        console.log("[HeroAdmin] GET /leadadmin/all-content response", res?.status, res?.data);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6">
      <div className="mb-4">
        <div className="text-2xl font-semibold">Banner/Herothis is lead admin pag e</div>
      ...
      <div className="text-xs text-muted-foreground">Changes update live via backend. No local storage used.</div>
    </div>
  );
};
