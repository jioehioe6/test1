...
const PhotoGalleryAdmin = () => {
  ...
  useEffect(() => {
    const load = async () => {
      console.log("[PhotoGalleryAdmin] GET /leadadmin/gallery - loading...");
      try {
        const res = await api.get("/leadadmin/gallery");
        console.log("[PhotoGalleryAdmin] GET /leadadmin/gallery response", res?.status, res?.data);
        const doc = res?.data;
  ...
  const savePhotos = async (updatedPhotos: PhotoGalleryItem[]) => {
    ...
    try {
      const res = await api.put("/leadadmin/gallery", { galleryImages });
      console.log("[PhotoGalleryAdmin] PUT /leadadmin/gallery response", res?.status, res?.data);
